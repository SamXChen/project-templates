...
import { ParserOptions, transformSync } from '@babel/core'
interface Options {
 ...
 * @type {import('.').default}
 */
function reactRefreshPlugin(opts?: Options) {
  ...
  return {
    ...
    transform(code, id, ssr) {
      ...
      if (opts && opts.parserPlugins) {
        parserPlugins.push(...opts.parserPlugins as any)
      }

      if (/\$RefreshReg\$\(/.test(code)) {
        // component code has been injected
        return code
      }
...
reactRefreshPlugin.prembleCode = preambleCode
